...
Description:  Makes portfolio for engineers
License:      GPL2
...
*/
function create_post_type_portfolio_item() {
	$labels = array(
...
*/
function create_publication_type_taxonomy() {
...
function add_taxonomy_submenu_page() {
	add_submenu_page('engineering-portfolio', __('Skills'), __('Skills'), 'manage_options', 'edit-tags.php?taxonomy=skill', null );	
}

add_action( 'init', 'create_post_type_portfolio_item', 40 );
add_action( 'init', 'create_post_type_publications',   30 );
add_action( 'init', 'create_post_type_project',        20 );
add_action( 'init', 'create_skill_taxonomy',           50 );
add_action( 'init', 'create_publication_type_taxonomy',60 );
add_action( 'admin_menu', 'add_taxonomy_submenu_page' );
/*
  adds custom metabox (area with input fields) for the admin page for the 'project' custom post type
*/ 
function add_meta_boxes_callback_project(){
	// parameters:
...
add_action("add_meta_boxes", "add_meta_boxes_callback_portfolio_item");


/*
  adds custom metabox (area with input fields) for the admin page for all custom post types
*/ 
function add_meta_boxes_callback_featured(){
...
add_action("add_meta_boxes", "add_meta_boxes_callback_featured");
 ...
 */
function add_meta_box_callback_render_project_details($post, $args) {
 ...
 */
function add_meta_box_callback_render_publication($post, $args) {
	/* The below will populate it with the existing database entry to edit existing content*/
	if ($post != null) {
...
*/
function add_meta_box_callback_render_portfolio_item($post, $args) {
	/* The below will populate it with the existing database entry to edit existing content*/
	if ($post != null) {
...
function add_meta_box_callback_render_featured($post, $args) {
	$DEFAULT = 1;
	if ($post != null) {
...
/*
*/
function add_meta_box_callback_render_related_publications_for_project($post) {
...
function get_related_publication_list_item($project_publication_id) {
	ob_start();
	?>
 ...
 */
function add_meta_box_callback_render_related_publications($post, $args) {
...
*/ 
function update_post_callback_custom_post_type_project($post_id, $post){
...
add_action('post_updated', 'update_post_callback_custom_post_type_project', 10, 2);


/*
  Saves the values entered in the custom metaboxes for the publication custom post type
*/
function update_post_callback_custom_post_type_publication($post_id, $post){
...
add_action('post_updated', 'update_post_callback_custom_post_type_publication', 10, 2);

/*
  Saves the values entered in the custom metaboxes for the portfolio_item custom post type
*/
function update_post_callback_custom_post_type_portfolio_item($post_id, $post){
...
add_action('post_updated', 'update_post_callback_custom_post_type_portfolio_item', 10, 2);


function update_post_callback_featured($post_id, $post, $update){
...
add_action('post_updated', 'update_post_callback_featured', 10, 3);
...
function associate_publication_with_project($post_id_project, $post_id_publication) {
...
function disassociate_publication_with_project($post_id_project, $post_id_publication) {
	$success = TRUE;
	$project_publication_id_list = maybe_unserialize(get_post_meta($post_id_project, "project_publication_id_list", true));	
	if (is_array($project_publication_id_list) && sizeof($project_publication_id_list) > 0) {
		//remove from array
...
function update_publication_post_meta($post_id, $new_publication) {
...
function store_publication_file($post_id, $dummy) {
...
*/
function publication_add_new_action() {
...
/*
	This is the ajax responder - it gets input from client-side javascript and sends back a response.
*/
function publication_add_existing_action() {
...
*/
function publication_remove_action() {
	$post_ID         = intval($_POST['post_ID']);
...
*/
add_action('wp_ajax_nopriv_publication_add_new_action', 'publication_add_new_action');
add_action('wp_ajax_publication_add_new_action', 				'publication_add_new_action');

add_action('wp_ajax_nopriv_publication_add_existing_action', 'publication_add_existing_action');
add_action('wp_ajax_publication_add_existing_action', 			 'publication_add_existing_action');

add_action('wp_ajax_nopriv_publication_remove_action', 'publication_remove_action');
add_action('wp_ajax_publication_remove_action', 			 'publication_remove_action');


// Add featured post column in admin panel
function manage_posts_columns_callback_featured($columns) {
...
// Add featured post column in admin panel
function manage_posts_custom_columns_callback_featured($column_name, $post_ID) {
...
add_filter('manage_project_posts_columns', 'manage_posts_columns_callback_featured');
add_action('manage_project_posts_custom_column', 'manage_posts_custom_columns_callback_featured', 10, 2);
// Add featured post column in admin panel for publication custom post type
add_filter('manage_publication_posts_columns', 'manage_posts_columns_callback_featured');
add_action('manage_publication_posts_custom_column', 'manage_posts_custom_columns_callback_featured', 10, 2);
// Add featured post column in admin panel for portfolio_item custom post type
add_filter('manage_portfolio_item_posts_columns', 'manage_posts_columns_callback_featured');
add_action('manage_portfolio_item_posts_custom_column', 'manage_posts_custom_columns_callback_featured', 10, 2);
...
function post_edit_form_tag_callback_multipart_encoding() {

    echo ' enctype="multipart/form-data"';

}
add_action('post_edit_form_tag', 'post_edit_form_tag_callback_multipart_encoding');


add_action('update_post','update_post_callback_publication_upload_file', 10, 2);
